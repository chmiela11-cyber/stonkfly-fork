...
The deadband must exceed the largest fee one order can book (order limit times fee reserve, 0.20 USDC at the defaults). Below that, executing a trade crosses the threshold on its own: every fill schedules an aversive pulse whatever the price then does, and the signal reports the cost of trading rather than the direction of the move. Fees still enter the change and still bias it downward; they no longer determine the stimulus by themselves.
