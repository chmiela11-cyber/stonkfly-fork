...
**Addendum, 2026-09-15.** The run above used the former defaults: a 60-second observation interval equal to the order cooldown, and a 0.01 USDC reinforcement deadband. A later paper run on a server reproduced both consequences over eleven observations. Consecutive proposals were vetoed by the cooldown roughly half the time, depending on integration jitter, and every fill scheduled an aversive pulse on its own, because one order books about 0.0585 USDC in modeled fees against a 0.01 USDC threshold. Equity fell 0.257 USDC over that window while fees accounted for 0.294 USDC of it, so the observed loss was trading cost rather than an adverse market. The defaults are now a 65-second interval against a 60-second cooldown, and a 0.10 USDC deadband. A first attempt at 0.25 was measured and rejected: across seventeen observations it scheduled no pulse at all, so it silenced the signal rather than cleaning it. Live runs must raise it above the preview fee ceiling and the CLI refuses to start otherwise. Neither change has been shown to improve decisions; they remove two artifacts that made the existing signal uninterpretable. Local tests after the change: **48 passed, 1 skipped** (the opt-in full-connectome test was not run).
