...
- Observations are sampled every 65 seconds, deliberately longer than the 60-second cooldown. An attempt follows a neural integration of variable duration, so consecutive attempts are one interval apart plus that jitter; an interval equal to the cooldown vetoes a proposal whenever the current integration finishes sooner than the previous one. `interval_margin_seconds` sets the required headroom and configuration that does not clear it is rejected.
